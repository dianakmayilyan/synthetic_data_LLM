...
#
#
class FixedVocabularyLogitsProcessor(LogitsProcessor):
        def __init__(self, vocab_tokens):
            self.vocab_tokens = set(vocab_tokens)
            
        def __call__(self, input_ids, scores):
            mask = torch.full_like(scores, float('-inf'))
            for token_id in self.vocab_tokens:
                mask[:, token_id] = 0
            return scores + mask
        ###ADDED END
...
class Tabula:
    ...
    def __init__(self, llm: str, experiment_dir: str = "trainer_tabula", epochs: int = 100,
        ...
        self.conditional_col_dist = None
            ...
            #
            #
        self.internet_name_vocab = ['3PLAY PREMIUM HRZ', 'RED INTERNET & PHONE CABLE U', 'VF CABLEMAX',
                                    'GIGAZUHAUSE KABEL', '3PLAY PREMIUM', 'GIGAZUHAUSE TREUE C', 'MMA',
                                    'GIGAZUHAUSE TREUE B', 'VF RIP U-TREUE', '3PLAY JUMP',
                                    'GIGAZUHAUSE CABLEMAX', '2PLAY PLUS', 'EAZY',
                                    'GIGAZUHAUSE TREUE D', '3PLAY SMART', 'INTERNET COMFORT',
                                    '3PLAY TREUE', '3PLAY FLY', 'INTERNET PREMIUM', '2PLAY',
                                    'EAZY FLEX', '1PLAY', '3PLAY TREUE HRZ', '3PLAY COMFORT',
                                    'VF CABLEMAX FULL FLEX', '3PLAY PLUS', '128KBIT', 'TELEFON HSI',
                                    '3PLAY TREUE GENRE', '2PLAY TREUE', 'INTERNET', 'TREUEPLUS',
                                    'HSI HZ BOX ONLY', '2PLAY PREMIUM', 'RIP CABLE U TREUE',
                                    '2PLAY JUMP', '3PLAY + PAY', '3PLAY', '3PLAY START',
                                    'GIGAZUHAUSE TREUE', '2PLAY FLY', 'GIGAZUHAUSE CABLEMAX EB',
                                    '3PLAY HD BOX', '3PLAY PLUS MTA', '2PLAY START',
                                    '3PLAY PREMIUM MTA', 'INTERNET START HZ TV', '2PLAY COMFORT',
                                    '2PLAY TREU', '3PLAY VORTEIL', 'MMA MB', '1PLAY MAX',
                                    '3PLAY SMART HRZ', '3PLAY HD BOX + ALLSTARS',
                                    '3PLAY + HD BOX + PAY', '3PLAY TREUE MODEM', '3PLAY PLUS HRZ',
                                    'TREUEPLUS BASIS']
        ## _________
    ...
    #
    #
                if column == "internet_product_name" :
                    label_encoder.fit(self.internet_name_vocab)
                else:
    ...
    def decode_categorical_column(self, data: pd.DataFrame):
        ...
        for i in range(len(self.label_encoder_list)):
            ...
            #
            #
            if self.label_encoder_list[i]['column'] == 'internet_product_name':
                le.classes_ = np.array(self.internet_name_vocab)
    ...
    def sample(self, n_samples: int,
        ...
        #
        #
        fixed_vocab_tokens = self.tokenizer.convert_tokens_to_ids(self.internet_name_vocab)
        # ADDED
        ...
        with tqdm(total=n_samples) as pbar:
            ...
            while n_samples > df_gen.shape[0]:
                ...
                #
                #
                logits_processor = LogitsProcessorList([FixedVocabularyLogitsProcessor(fixed_vocab_tokens)])
                ## ADDED END
